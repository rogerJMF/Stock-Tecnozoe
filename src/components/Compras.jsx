...
import { Search, User, Smartphone, Calendar, Truck, Filter } from 'lucide-react';
...
export default function Compras() {
  ...
  // 1. Procesar los datos y aplanarlos para facilitar el renderizado
  ...
  return (
    <div className="container mx-auto p-4 pt-24 text-white">
      ...
      >
        ...
        {/* Buscador con estilo tipo "Paleta de comandos" */}
        <div className="relative w-full md:w-72 lg:w-96 group">
          ...
          <input 
            type="text" 
            placeholder="Buscar por producto, IMEI, proveedor, fecha..." 
            className="w-full pl-10 pr-10 py-3 bg-slate-800/70 backdrop-blur-sm border border-white/10 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all shadow-lg"
            value={searchTerm}
          ...
          {searchTerm && (
            <button 
              ...
              className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-white transition-colors text-xs bg-slate-700/50 p-1 rounded-full hover:bg-slate-600"
            >
              ✕
            </button>
          )}
      ...
      {comprasFiltradas.length === 0 ? (
        ...
        >
          ...
          <p className="text-xl font-semibold text-slate-300">
            {searchTerm.trim() ? 'No hay compras que coincidan con tu búsqueda' : 'Aún no hay compras registradas'}
          </p>
          <p className="text-sm text-slate-500">
            {searchTerm.trim() ? 'Intenta con otro término o limpia el filtro.' : 'Las compras aparecerán aquí automáticamente.'}
          </p>
          {searchTerm.trim() && (
            <button onClick={() => setSearchTerm('')} className="mt-2 text-blue-400 hover:text-blue-300 underline text-sm">Limpiar búsqueda</button>
          )}
        </motion.div>
      ) : (
        <motion.div 
