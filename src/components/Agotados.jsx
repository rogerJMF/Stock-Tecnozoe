...
import { PackageX, Cpu, MemoryStick, HardDrive } from 'lucide-react';
...
export default function Agotados() {
  ...
  Object.keys(datosAgrupados).forEach(marcaKey => {
    const modelosFiltrados = Object.values(datosAgrupados[marcaKey].modelos)
      .filter(grupo => grupo.stockTotal === 0);
    ...
    <div className="container mx-auto p-4 pt-24 text-white">
      ...
      >
        <h1 className="text-3xl font-bold bg-gradient-to-r from-red-400 to-orange-400 bg-clip-text text-transparent flex items-center gap-3">
          <PackageX className="w-8 h-8 text-red-400" /> Productos Agotados
        </h1>
      ...
      {Object.keys(productosAgotados).length === 0 ? (
        <motion.div 
          ...
          className="bg-slate-800/40 backdrop-blur-md border border-white/10 rounded-3xl p-16 text-center text-slate-400 flex flex-col items-center justify-center gap-4 min-h-[400px] shadow-2xl"
        >
          <PackageX className="w-20 h-20 text-slate-500/50" />
          <p className="text-2xl font-semibold text-slate-300">¡No hay productos agotados en este momento!</p>
          <p className="text-sm text-slate-500">Revisa el inventario disponible en la sección de <span className="text-blue-400">Inicio</span>.</p>
        </motion.div>
      ) : (
        Object.keys(productosAgotados).sort().map(marcaKey => {
          ...
          return (
            <motion.div 
              ...
              initial={{ opacity: 0, y: 20 }} 
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 * Object.keys(productosAgotados).indexOf(marcaKey) }}
              className="mb-10"
            >
              <h2 className="text-xl font-bold text-red-300 mb-4 uppercase tracking-wider flex items-center gap-2 border-b border-red-500/20 pb-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {productosAgotados[marcaKey].modelos.map((grupo, idx) => (
                  <motion.div
                    ...
                    className="bg-slate-800/60 backdrop-blur-sm border border-white/10 rounded-2xl p-6 shadow-lg relative overflow-hidden group"
                  >
                    ...
                    <div className="relative z-10 flex flex-col h-full justify-between">
                      <div>
                        <div className="flex items-start justify-between mb-2">
                          <h3 className="text-xl font-bold text-white tracking-tight">{grupo.nombreModelo}</h3>
                          <span className="bg-red-900/50 text-red-300 text-[10px] px-2 py-0.5 rounded-full border border-red-500/30 uppercase font-bold">
                            AGOTADO
                          </span>
                        </div>
                        
                        <p className="text-slate-400 text-sm mb-3 font-mono">{grupo.modeloRef}</p>
                        
                        <div className="flex flex-wrap gap-2 mt-2">
                          <span className="flex items-center gap-1.5 bg-slate-700/50 px-3 py-1.5 rounded-full text-xs border border-white/5 text-slate-300">
                            <MemoryStick className="w-3 h-3 text-blue-400" /> {grupo.ram}
                          </span>
                          <span className="flex items-center gap-1.5 bg-slate-700/50 px-3 py-1.5 rounded-full text-xs border border-white/5 text-slate-300">
                            <HardDrive className="w-3 h-3 text-purple-400" /> {grupo.almacenamiento}
                          </span>
                        </div>
                      </div>
                    </div>
