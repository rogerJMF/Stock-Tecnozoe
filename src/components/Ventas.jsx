...
export default function Ventas() {
  ...
  const ventasProcesadas = useMemo(() => {
    return movimientos
      ...
      .flatMap(m => m.detalles.map(d => ({
        ...
        proveedorOrigen: d.UnidadInventario.proveedorOrigen?.nombre || 'Sin proveedor origen'
      })));
  }, [movimientos]);
  ...
  const ventasFiltradas = useMemo(() => {
    ...
    return ventasProcesadas.filter(v => 
      ...
      v.proveedorOrigen.toLowerCase().includes(lowerSearch) ||
      (v.cliente?.nombre || '').toLowerCase().includes(lowerSearch)
    );
    ...
    <div className="container mx-auto p-4 pt-24 text-white">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div className="flex items-center gap-3">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-green-400 to-emerald-400 bg-clip-text text-transparent flex items-center gap-3">
            <Tag className="w-8 h-8 text-green-400" /> Ventas Realizadas
          </h1>
          <span className="bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-sm border border-green-500/30 shadow-[0_0_15px_rgba(34,197,94,0.1)]">
      ...
      {ventasFiltradas.length === 0 ? (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-800/40 backdrop-blur-md border border-white/10 rounded-3xl p-16 text-center text-slate-400 flex flex-col items-center justify-center gap-4 min-h-[400px] shadow-2xl">
          <Tag className="w-16 h-16 text-slate-500/50" />
          <p className="text-xl font-semibold text-slate-300">{searchTerm.trim() ? 'No hay ventas que coincidan' : 'Aún no hay ventas registradas'}</p>
          {searchTerm.trim() && <button onClick={() => setSearchTerm('')} className="mt-2 text-green-400 hover:text-green-300 underline text-sm">Limpiar búsqueda</button>}
        </motion.div>
      ) : (
                ...
                <div className="relative z-10 flex flex-col h-full justify-between gap-2">
                  <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-start gap-2">
                      <h3 className="text-lg font-bold text-white tracking-tight truncate group-hover:text-green-300 transition-colors">{v.producto}</h3>
                      <span className="bg-slate-700/80 border border-white/10 text-slate-300 text-[10px] px-2 py-0.5 rounded-full uppercase font-bold shrink-0 group-hover:bg-green-900/50 group-hover:text-green-300 group-hover:border-green-500/30 transition-all">{v.marca}</span>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-1">
                      <span className="flex items-center gap-1 bg-slate-700/50 px-2 py-0.5 rounded-full text-[10px] border border-white/5 text-slate-300"><Palette className="w-3 h-3 text-green-300" /> {v.color}</span>
